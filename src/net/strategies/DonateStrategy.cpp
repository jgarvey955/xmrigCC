    ...
    m_listener(listener)
{
    ...
    m_pools.emplace_back(kDonateHost, kDonatePort, kDonateUser, nullptr, nullptr, 0, false, false, mode);
...
void xmrig::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    ...
    params.AddMember("url", Value(m_pools[0].url().data(), allocator), allocator);
...
xmrig::IClient *xmrig::DonateStrategy::createProxy()
{
    ...
    Pool pool(client->pool().proxy().isValid() ? client->pool().host() : client->ip(), client->pool().port(), kDonateUser,
              client->pool().password(), client->pool().spendSecretKey(), 0, false, client->isTLS(), Pool::MODE_POOL);
...
void xmrig::DonateStrategy::setParams(rapidjson::Document &doc, rapidjson::Value &params)
{
    ...
    params.AddMember(StringRef(Pool::kCoin), StringRef("salvium"), allocator);
